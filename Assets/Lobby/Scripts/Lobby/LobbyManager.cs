...
namespace Prototype.NetworkLobby
{
    public class LobbyManager : NetworkLobbyManager 
    {
        ...
        {
            /*Debug.Log("create game player of "+playerControllerId);
            Debug.Log(conn.playerControllers.Count);
            //return Instantiate(PropPlayer) as GameObject;
            foreach (GameObject obj in GameObject.FindGameObjectsWithTag("PlayerInfo"))
            {
                LobbyPlayer playerInfo = obj.GetComponent<LobbyPlayer>();
                if (playerInfo.isLocalPlayer)
                    if(playerInfo.playerColor.Equals(Color.red))
                        return Instantiate(PropPlayer) as GameObject;
                    else
                        return Instantiate(HunterPlayer) as GameObject;
            }
            Debug.Log("Error en LobbyManager::OnLobbyServerCreateGamePlayer");*/
           /* foreach(PlayerController pc in conn.playerControllers)
            {
                Debug.Log(playerControllerId+" - Player controller id: "+pc.playerControllerId);
                Debug.Log(conn.playerControllers[0].gameObject.name);
            }*/
            //GameObject player = Instantiate<GameObject>(PropPlayer);
        ...
        public override bool OnLobbyServerSceneLoadedForPlayer(GameObject lobbyPlayer, GameObject gamePlayer)
        {
            ...
            int i = 0;
            //Debug.Log("Length: "+lobbySlots.Length);
            foreach (NetworkLobbyPlayer player in lobbySlots)
            {
                if (player == null)
                    break;
                Debug.Log("lobbySlots" + i + " " + player.connectionToClient + " " + player.isLocalPlayer);
                i++;
            }
            if (_lobbyHooks)
            {
